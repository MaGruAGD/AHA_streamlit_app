...
def well_plate_selector(key, title="Select Position", default_position="A1"):
    """
    Create a well plate selector component for Streamlit
    ...
    <body>
        <div class="plate-container">
            ...
            <div id="selected-display" class="selected-display">
                Selected: <span id="selected-position">{default_position}</span>
            </div>
        </div>
        ...
        <script>
                ...
                rows.forEach((row, rowIndex) => {{
                    ...
                    cols.forEach(col => {{
                        ...
                        if (position === selectedPosition) {{
                            ...
                            well.textContent = position;
                        }}
                        
                        well.addEventListener('click', () => selectWell(position));
            ...
            function selectWell(position) {{
                // Remove selection from previous well
                const previousWell = document.getElementById('well-' + selectedPosition);
                if (previousWell) {{
                    previousWell.classList.remove('selected');
                    previousWell.textContent = '';
                }}
                
                // Select new well
                const newWell = document.getElementById('well-' + position);
                if (newWell) {{
                    newWell.classList.add('selected');
                    newWell.textContent = position;
                }}
